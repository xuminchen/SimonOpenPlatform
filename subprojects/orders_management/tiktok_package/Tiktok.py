...
class TiktokApp:
    def __init__(self):
        ...
        self.shop_map = config.get("shops", {})
        if not self.app_key or not self.app_secret:
            raise ValueError("Missing orders_management.tiktok_package.app_key/app_secret in config/api_credentials.json")

    def sign_param(self, method, timestamp, param):
        """
        :param method: 接口名称
        :param timestamp: 时间戳
        :param param: 请求参数
        :return: 请求参数的加密结果signature
        """

        # param必须按照字典的键进行排序，再序列化，并禁用Html Escape（separators=(',', ':')）
    ...
    def fetch_resp(self, method, param, access_token, times=5):
        ...
        # sign获取
        sign = self.sign_param(method, timestamp, param)
    ...
    def get_token(self, shop_id):
        ...
        signature = self.sign_param(method, timestamp, param)
